...
function ReadingBar({ pct }: { pct: number }) {
  return (
    <div className="absolute top-0 left-0 right-0 h-0.5 z-20" style={{ background: C.border }}>
    ...
    <section>
      ...
      <button
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px]" style={{ color: C.faint }}>{readMin} min</span>
          <span className="text-xs transition-transform duration-200"
            style={{ color: C.muted, display: "inline-block", transform: open ? "rotate(0deg)" : "rotate(-90deg)" }}>
            ▾
          </span>
        </div>
      </button>
      ...
      <div className="rounded-lg overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
        <div className="grid grid-cols-3 px-3 py-2"
          ...
          {["Service", "CAP Choice", "Reasoning"].map(h => (
            <span key={h} className="text-[9px] font-bold uppercase tracking-widest" style={{ color: C.faint }}>{h}</span>
          ))}
        </div>
        {CAP_ROWS.map((row, i) => (
          <div key={i} className="grid grid-cols-3 px-3 py-3 gap-2"
            style={{ borderBottom: i < CAP_ROWS.length - 1 ? `1px solid ${C.border}18` : undefined }}>
            <span className="text-xs font-medium" style={{ color: C.text }}>{row.service}</span>
            <div>
            ...
            <p className="text-[10px] leading-relaxed" style={{ color: interviewMode ? C.text2 : C.muted }}>
...
function ComparisonCard({ a, b }: { a: { label: string; value: string; color: string }; b: { label: string; value: string; color: string } }) {
  return (
    <div className="grid grid-cols-2 gap-px rounded-lg overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
      <div className="p-3" style={{ background: C.card2 }}>
        <p className="text-[8px] font-bold uppercase tracking-widest mb-0.5" style={{ color: a.color }}>{a.label}</p>
        <p className="text-[10px]" style={{ color: C.text2 }}>{a.value}</p>
      </div>
      <div className="p-3" style={{ background: C.card2, borderLeft: `1px solid ${C.border}` }}>
        <p className="text-[8px] font-bold uppercase tracking-widest mb-0.5" style={{ color: b.color }}>{b.label}</p>
        <p className="text-[10px]" style={{ color: C.text2 }}>{b.value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      {DB_ROWS.map((row, i) => {
        ...
        return (
          <div key={i} className="rounded-lg overflow-hidden" style={{ border: `1px solid ${isOpen ? C.border2 : C.border}` }}>
            <button onClick={() => setExpanded(isOpen ? null : row.store)}
              ...
              <div className="flex-1">
                <p className="text-sm font-bold" style={{ color: C.text }}>{row.store}</p>
                <p className="text-[10px]" style={{ color: C.muted }}>{row.usedFor}</p>
              </div>
              <div className="text-right hidden sm:block">
                <p className="text-[9px]" style={{ color: row.color }}>{row.latency}</p>
                <p className="text-[9px]" style={{ color: C.muted }}>{row.consistency}</p>
              </div>
            ...
            {isOpen && (
              <div className="px-4 pb-4 space-y-3">
                ...
                />
                <ComparisonCard
                  a={{ label: "Throughput", value: row.throughput, color: C.text2 }}
                  b={{ label: "Cost Profile", value: row.costProfile, color: C.text2 }}
                />
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-widest mb-1" style={{ color: C.faint }}>Why this wins</p>
                  <p className="text-[10px] leading-relaxed" style={{ color: C.text2 }}>{row.whyWins}</p>
                </div>
                <div className="rounded p-2.5" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)" }}>
                  ...
                  <p className="text-[10px] leading-relaxed" style={{ color: C.muted }}>{row.antiPattern}</p>
                </div>
              </div>
            )}
          </div>
...
function PushPullSection() {
  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
      <div className="grid grid-cols-4 px-3 py-2" style={{ background: C.card2, borderBottom: `1px solid ${C.border}` }}>
      ...
      {PUSH_PULL_ROWS.map((row, i) => {
        ...
        return (
          <div key={i} className="grid grid-cols-4 px-3 py-3 gap-2"
            style={{ borderBottom: i < PUSH_PULL_ROWS.length - 1 ? `1px solid ${C.border}18` : undefined }}>
            <div>
              <p className="text-xs font-medium" style={{ color: C.text }}>{row.name}</p>
              <p className="text-[9px]" style={{ color: C.muted }}>{row.usedAt}</p>
            </div>
            <div>
            ...
            <div>
              <p className="text-[10px]" style={{ color: C.text2 }}>{row.latency}</p>
              <p className="text-[9px]" style={{ color: C.muted }}>{row.staleness}</p>
            </div>
            <p className="text-[10px] leading-relaxed" style={{ color: C.muted }}>{row.tradeoff}</p>
          </div>
        );
      })}
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-sm font-bold" style={{ color: C.text }}>Benefits</p>
        </div>
        <div className="space-y-2">
          {MICROSERVICES_BENEFITS.map((b, i) => (
            <div key={i} className="rounded-lg p-3" style={{ background: C.card2, border: `1px solid ${C.border}` }}>
              ...
              <p className="text-[10px] leading-relaxed" style={{ color: C.text2 }}>{b.desc}</p>
            </div>
          ))}
        </div>
      </div>
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-sm font-bold" style={{ color: C.text }}>Costs</p>
        </div>
        <div className="space-y-2">
          {MICROSERVICES_COSTS.map((c, i) => (
            <div key={i} className="rounded-lg p-3" style={{ background: C.card2, border: `1px solid ${C.border}` }}>
              <p className="text-xs font-bold mb-1" style={{ color: "#ef4444" }}>{c.title}</p>
              <p className="text-[10px] leading-relaxed" style={{ color: C.text2 }}>{c.desc}</p>
            </div>
          ))}
        </div>
      </div>
...
export default function TradeoffsTab({ interviewMode }: { interviewMode: boolean }) {
  ...
  return (
    <div className="flex h-full overflow-hidden relative" style={{ background: C.bg }}>
      <ReadingBar pct={readPct} />

      {/* Sticky TOC */}
      <div className="w-44 shrink-0 hidden md:block pt-0.5" style={{ background: C.card, borderRight: `1px solid ${C.border}` }}>
        <div className="px-3 py-3" style={{ borderBottom: `1px solid ${C.border}` }}>
          <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: C.faint }}>Contents</p>
          <p className="text-[10px] mt-0.5" style={{ color: C.muted }}>~{TOTAL_MIN} min read</p>
        </div>
          ...
          {SECTIONS.map(s => (
            <button key={s.id} onClick={() => scrollTo(s.id)}
              ...
              style={{
                ...
                color: activeSection === s.id ? C.red : C.text2,
                borderLeft: `2px solid ${activeSection === s.id ? C.red : "transparent"}`,
              }}>
              <span>{s.label}</span>
              <span className="ml-1 text-[9px]" style={{ color: C.faint }}>{s.readMin}m</span>
            </button>
          ))}
        </nav>
        {readPct > 0 && (
          <div className="px-3 pb-3">
            <div className="h-1 rounded-full mt-2" style={{ background: C.border }}>
              <div className="h-full rounded-full" style={{ width: `${readPct}%`, background: C.red }} />
            </div>
            <p className="text-[8px] mt-1" style={{ color: C.faint }}>{Math.round(readPct)}% read</p>
          </div>
        )}
      </div>
      ...
      <div ref={containerRef} className="flex-1 overflow-y-auto px-5 pt-4 pb-10 space-y-8">
        <div className="flex items-center gap-3 mb-2">
          <p className="text-xs" style={{ color: C.muted }}>~{TOTAL_MIN} min read</p>
          {interviewMode && (
