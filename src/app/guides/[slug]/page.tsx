...
import { getGuidePosts, getPost } from "@/lib/posts";
import { MDXRemote } from "next-mdx-remote/rsc";
import Link from "next/link";

export async function generateStaticParams() {
  return getGuidePosts().map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = getPost("guide", slug);
  if (!post) return {};
...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16">
      <Link href="/guides" className="text-sm text-gray-500 hover:text-[var(--accent-light)] mb-8 inline-block">
        ← Back to guides
      </Link>
